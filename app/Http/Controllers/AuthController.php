...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        //facebook login link appear or not
        ...
        if (View::exists('public.'.ONE::getEntityLayout().'.auth.login')) {
            return view('public.'.ONE::getEntityLayout().'.auth.login', compact('facebook_login'));
        }
        return view('auth.login', compact('facebook_login'));
    }
            ...
            if (!empty(Session::get('SITE-CONFIGURATION')['recaptcha_site_key']) &&  !empty(Session::get('SITE-CONFIGURATION')['recaptcha_secret_key'])){
                if(!empty(Input::get('g-recaptcha-response'))){
                    if(!ONE::verifyRecaptcha(Session::get('SITE-CONFIGURATION')['recaptcha_secret_key'], Input::get('g-recaptcha-response'))){
                        return redirect()->back()->withErrors([trans('auth.verifyRegister') => trans('defaultRegister.captcha_login_error')])->withInput($request->except('password'));
                    }
                }else{
                    return redirect()->back()->withErrors([trans('auth.verifyRegister') => trans('defaultRegister.captcha_login_error')])->withInput($request->except('password'));
                }
    ...
    public function verifyRegisterAndLogin(RegisterRequest $request)
    {
        try {
            if (!empty(Session::get('SITE-CONFIGURATION')['recaptcha_site_key']) &&  !empty(Session::get('SITE-CONFIGURATION')['recaptcha_secret_key'])){
                if(!empty(Input::get('g-recaptcha-response'))){
                    if(!ONE::verifyRecaptcha(Session::get('SITE-CONFIGURATION')['recaptcha_secret_key'], Input::get('g-recaptcha-response'))){
                        return redirect()->back()->withErrors([trans('auth.verifyRegisterAndLogin') => trans('defaultRegister.captcha_login_error')])->withInput($request->except('password'));
                    }
                }else{
                    return redirect()->back()->withErrors([trans('auth.verifyRegisterAndLogin') => trans('defaultRegister.captcha_login_error')])->withInput($request->except('password'));
                }
            ...
            if(!empty(Session::get("SITE-CONFIGURATION.boolean_no_email_needed")) && $request->get("withoutemail",0)==1) {
                $request["email"] = $this->generateFakeEmail($request['name'],$request['surname']);
            ...
            Session::set('new_registration',true);
            Session::flash('message', trans('register.success'));
            //OneLog::info("Registration Success.");
...
//          Update User Level
     ...
     * @return string
     */
    public static function generateFakeEmail($name, $surname)
    {
        $faker = Faker::create();
