...
<div style="margin:10px 30px;">
    <div class="row">
        <div class="col-12">
            <div class="chart-download-wrapper{{ $parameterId }}">
                <a id="downloadCSV{{ $parameterId }}"  class="btn btn-flat btn-blue pull-right">
                    <i class="fa fa-file-excel-o" aria-hidden="true"></i> {{ trans('privateCbsVoteAnalysis.download_csv') }}
                </a>
                <a id="downloadImage{{ $parameterId }}"  class="btn btn-flat btn-blue pull-right" style="margin-right:5px;">
...
<!-- Chart -->
<div id="statistics_by_date{{ $parameterId }}" style="height:600px;" class="margin-bottom-30"></div>

<!-- Canvas for downloading chart -->
<canvas id="canvas_statistics_by_date{{ $parameterId }}" height="600" style="display: none;"></canvas>

<script>
    $( document ).ready(function() {
        // Date Range pickers
        $('input[name="dr_vote_analysis_date2{{ $parameterId }}"]').daterangepicker({
            timePicker: false,
    ...
    var statistics_by_date_data{{ $parameterId }} = [];

    // Total
    // if ($('#total{{ $parameterId }}').is(":checked")) {
    @php
        $arrayChartLinesKey = [];
    @endphp
...
{{--                    @if( (!empty($value) &&  empty($selectLineToView))
    ...
    var start = new Date($("#daterangepicker{{ $parameterId }}").val().split(" - ")[0]);
    var end = new Date($("#daterangepicker{{ $parameterId }}").val().split(" - ")[1]);
    ...
    // statistics_by_date_data{{ $parameterId }} = statistics_by_date_data{{ $parameterId }}.concat(statistics_total_result);
    statistics_by_date_data{{ $parameterId }} =  statistics_total;
    // }


    // Setting Y range - bug fix when values are 0's
    var chartRange = [0, 10];
    for (var i = 0; i < statistics_by_date_data{{ $parameterId }}.length; i++) {
        if (statistics_by_date_data{{ $parameterId }}[i].{!! trim(preg_replace('/\s\s+/', ' ',trans('privateCbsVoteAnalysis.votes'))) !!} != 0) {
            chartRange = false;
        }
    }
    ...
    // Chart D3plus
    d3plus.viz()
        .container("#statistics_by_date{{ $parameterId }}")
        .data(statistics_by_date_data{{ $parameterId }})
        .type("line")
        .id("name")
        .color({
            "value": "name"
    ...
    if (statistics_by_date_data{{ $parameterId }}.length == 0) {
        var warningMessage = "{!! trim(preg_replace('/\s\s+/', ' ', trans('privateCbsVoteAnalysis.no_data_Available'))) !!}";
        $('#statistics_by_date{{ $parameterId }}').html('<div class="chartMessage"><div><i class="fa fa-eye-slash" aria-hidden="true"></i> ' + warningMessage + '</div></div>');
        toastr.warning(warningMessage);
        $(".chart-download-wrapper{{ $parameterId }}").remove();
    } else {
        // Export data for CSV (javascript)
        $("#downloadCSV{{ $parameterId }}").click(function () {
            var d = new Date();
            ...
            downloadCSV(statistics_by_date_data{{ $parameterId }}, filename);
        });
        // Download chart to PNG (javascript)
        $("#downloadImage{{ $parameterId }}").click(function () {
            var d = new Date();
            var suffix_name = d.getFullYear() + "_" + (1 + d.getMonth()) + "_" + d.getDate() + "_" + d.getHours() + "_" + d.getMinutes() + "_" + d.getSeconds();
            var filename = "statistics_by_date_" + suffix_name + ".png";
            $("#canvas_statistics_by_date{{ $parameterId }}").attr("width", $("#statistics_by_date{{ $parameterId }} #d3plus").width());
            $("#canvas_statistics_by_date{{ $parameterId }}").attr("height", $("#statistics_by_date{{ $parameterId }} #d3plus").height());
            var svg = document.querySelector('#statistics_by_date{{ $parameterId }} svg');
            var canvas = document.getElementById('canvas_statistics_by_date{{ $parameterId }}');
