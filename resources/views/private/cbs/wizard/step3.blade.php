<div class="card-header card-header-gray">
    <i>{!! trans("privateCbs.addParameter") !!}</i>
    <a href="#" onclick="parametersView()" class="btn btn-flat btn-info2 btn-sm margin-right-5 pull-right" title="Create parameter"  data-original-title="Create parameter">
...
<div id="parametersGroup">
    ...
    <div class="dd" id="nestable">
        <ol id="parametersOrderedList" class="dd-list">
            @if(isset($cbTemplateParameters))
                @foreach($cbTemplateParameters as $parameter)
                    <li id="parameterItemId_{{$parameter->id}}" class="dd-item nested-list-item">
                        ...
                        <div class="nested-list-content">
                            ...
                            {{--<a style='margin-top:-6px;' onclick="removeParameterItem({{$parameter->id}})"  class='btn btn-flat btn-danger btn-sm' data-toggle='tooltip' data-delay='{&quot;show&quot;:&quot;1000&quot;}' title='Delete' data-original-title='Delete'><i class='fa fa-remove'></i></a>--}}
                            {{--@endif--}}
                            {{--</div>--}}
                        </div>
                    </li>
...
<script>
    ...
    function parametersView(){
        console.log($("#cb_key").text())
        @if(isset($cbKey))
        $.ajax({
            url: '{{action("CbsController@moderateRouting", ['type' => $type, 'action' => 'create', 'step' => 'param'])}}',
            method: 'get',
            data: {
                cbKey: '{{ $cbKey }}',
                _token: "{{ csrf_token()}}"
...
$.ajax({
    ...
    function showParameter(id){
        @if(isset($cbKey))
        $.ajax({
            url: '{{action("CbsController@moderateRouting", ['type' => $type, 'action' => 'show', 'step' => 'param'])}}',
            method: 'get',
            data: {
                cbKey: '{{ $cbKey }}',
                parameterId : id,
...
$.ajax({
    ...
    function addNewParameterModal(){
        ...
        $.ajax({
            url: '{{action("CbsController@addModalParameter")}}',
            method: 'POST',
            data: {
    ...
    function addNewParameterTemplate2(parameterCounter){
        $.ajax({
            url: '{{action("CbsController@addParameterTemplateSelection")}}',
            method: 'POST',
            data: {
                parameterCounter: parameterCounter,
                _token: "{{ csrf_token()}}"
    ...
    function addNewParameterItem2(parameterCounter){
        $.ajax({
            url: '{{action("CbsController@addParameter")}}',
            method: 'POST',
            data: {
                parameterCounter: parameterCounter,
                _token: "{{ csrf_token()}}"
    ...
    function removeOption(parameterCounter,id){
        ...
        if(numInputs <2){
            toastr.error("{!! preg_replace( "/\r|\n/", "", htmlentities(trans("privateCbs.oneOptionRequired!"),ENT_QUOTES)) !!}");
            return false;
        }
    ...
    function addNewOption(parameterCounter,val) {
        ...
        if(newOptionsDiv.length > 0){
            optionIndex = $('#'+newOptionsDiv[0].id +' > div').size() + 1;
        }
        for(var i=0 ;i< newOptionsDiv.length; i++){
            ...
            html += '<input class="form-control" id="optionsNew_'+parameterCounter+'" placeholder="{!! trans("privateCbs.optionValue") !!}" value="'+val+'" '+required+' name="optionsNew_'+parameterCounter+'['+optionIndex+']['+lang+']" type="text">';
            html += '</div>';
            html += '<div class="btn-group" style="margin-top:5px;margin-bottom:10px;">';
    ...
    function chooseTemplate(template,parameterCounter){
        $.ajax({
            url: '{{action("CbsController@addParameterTemplate")}}',
            method: 'POST',
            data: {
                parameterCounter: parameterCounter,
                template: template,
        ...
        if(booleanVar) {
            $(".newOptionsDiv_"+counter+" input").each(function(){
                ...
                if(optRequired && $(this).val()== ""){
                    toastr.error("{!! preg_replace( "/\r|\n/", "", htmlentities(trans("privateCbs.pleaseFillAllOptionsOnTab!"),ENT_QUOTES)) !!} #4!");
                    submitOk = false;
                    return false;
                }
            });
    ...
    function showModalParameter(parameterId){
        $.ajax({
            url: '{{action("CbsController@getParameter")}}',
            method: 'POST',
            data: {
                parameter_id: parameterId,
                _token: "{{ csrf_token()}}",
                cbKey : "{{ isset($cb) ? $cb->cb_key : "" }}"
            },
            success: function(response){
                if(response != 'false'){
