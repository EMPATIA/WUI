...
@section('content')
    @php
    $form = ONE::form('contents', trans('privateNews.details'), 'cm', $type)
    ...
    {!! Form::hidden('type', isset($type) ? $type : null) !!}
    ...
    @if(ONE::actionType('contents') == 'show')
        ...
        <div style="margin-bottom: 10px;">
            @if($activeVersion != $version)
                {!! Form::button('<i class="fa fa-check"></i>&nbsp;' . trans('privateNews.activateVersion'), ['class' => 'btn btn-flat btn-success pull-right', 'onclick' => "location.href='".action('ContentsController@activateVersion', [$content->content_key, $version])."'" ]) !!}
            @endif
            {!! Form::button('<i class="fa fa-eye"></i>&nbsp;' . trans('privateNews.preview'), ['class' => 'btn btn-flat btn-info ', 'data-delay' => '{"show":"1000"}', 'data-toggle' => 'tooltip', 'data-original-title' => trans('form.preview'), 'target' => '_blank', 'onclick' => 'window.open("'  . URL::action('PublicContentsController@previewPage', [$content->content_key, $version], false) .'")' ]) !!}


            @if(ONE::actionType('contents') == 'show' && $activeVersion == $version)
                @if(!$content->published)
                    {!! Form::button('<i class="fa fa-check"></i>&nbsp;' . trans('privateNews.publish'), ['class' => 'btn btn-flat btn btn-success pull-right ', 'data-delay' => '{"show":"1000"}', 'data-toggle' => 'tooltip', 'data-original-title' => trans('form.publish'), 'onclick' => "location.href='".action('ContentsController@publish', $content->content_key)."'" ]) !!}
    ...
    {!! Form::oneSelect('content_type_type', trans('privateNews.news_types'), isset($contentTypesSelect) ? $contentTypesSelect : null, '', isset($content->content_type_type->name) ? $content->content_type_type->name : '', ['class' => 'form-control', 'id' => 'content_type_type']) !!}
            ...
            @if(count($languages) > 0)
                @foreach($languages as $language)
                        ...
                        {!! Form::oneText($language->default == true ? 'required_title_'.$language->code : 'title_'.$language->code ,
                                           trans('privateNews.title'), isset($translations[$language->code]) ? $translations[$language->code]->title : null,
                        ...
                        {!! Form::oneTextArea($language->default == true ? 'required_summary_'.$language->code : 'summary_'.$language->code,
                                              trans('privateNews.summary'), isset($translations[$language->code]) ? $translations[$language->code]->summary : null,
                        ...
                        @if(ONE::actionType('contents') == 'show')
                            ...
                            @else
                                {!! Form::oneTextArea($language->default == true ? 'required_content_'.$language->code : 'content_'.$language->code,
                                                      trans('privateNews.content'), isset($translations[$language->code]) ? $translations[$language->code]->content : null,
                                                      ['class' => 'form-control content_page',
                                                      'id' => 'content_'.$language->code ]) !!}
                            @endif

                            {!! Form::oneText('link_'.$language->code , trans('privateNews.link'), isset($translations[$language->code]) ? $translations[$language->code]->link : null, ['class' => 'form-control', 'id' => 'link_'.$language->code ]) !!}
