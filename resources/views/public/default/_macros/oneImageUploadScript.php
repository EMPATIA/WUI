                        ...
                        if (result) {
                            result.toBlob(
                                function (blob) {
                                    ...
                                    var ola = up.addFile(file);

                                    $("#<?php echo $imageId; ?>").attr("src", "");
                                    $("#image-preview-files2").attr("src", "/images/default_image.gif");
                                    /* ... */
                                }
                            );
    ...
    var <?php echo $variable ?> = new plupload.Uploader({
        ...
        init: {
            ...
            FilesAdded: function(up, files) {
                ...
                if ($('#<?php echo $idModal; ?>').hasClass('show')) {
                    $('#<?php echo $idModal; ?>').modal('hide');
            ...
            FileUploaded: function(up, file, obj) {
                try {
                    ...
                    var jsonStringFile = '[{"id": '+jsonObj.result.id+', "code": "'+jsonObj.result.code+'", "name": "'+file.name+'" , "type": "'+file.type+'" , "size": '+file.size+', "description": ""  }]';
                    $("#<?php echo $objName; ?>").val(jsonStringFile);
...
//$("#upload-single-image").html("<p>Puta de foto estúpida!!!</p>")
                    $("#image-preview-files2").attr("src",'<?php echo $downloadPath; ?>?id='+jsonObj.result.id+'&code='+jsonObj.result.code);
                    $("#upload-single-image").css('height', 'auto');
                    $("#add_image_<?php echo $objName; ?>").modal('hide');
                } catch (e) {
                    console.log(e);
                }
            },
