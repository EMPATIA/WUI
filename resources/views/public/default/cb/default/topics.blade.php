...
@section('content')
    ...
    <div id="root">
        <login-levels id="modal_template" style="display:none">
            <div>
                <transition name="modal">
                    <div v-bind:class="{ hidden: !showModal }">
                            ...
                            <div class="modal-wrapper">
                                <div class="modal-container">
                                    ...
                                    <div class="modal-footer">
                                        <slot name="footer">
                                            <button class="cancel-btn" v-on:click="hide">
                                                <?php echo ONE::transCb("cb_close", $cbKey) ?>
                                            </button>
                                        </slot>
                                    </div>
                                </div>
        ...
        <cb id="cb-template" :cb-key="'{!! $cbKey !!}'" :cb-type="'{!! $type !!}'" :current-language="'{!! $currentLanguage !!}'" :logged-user="'{{ json_encode($currentUser) }}'" :default-image="'{{ $defaultImage }}'">
            <div>
                <div class="container">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-12 col-sm-12 col-md-5 offset-lg-1 col-lg-3 search-box">
                                                <div class="input-group">
                                                    <input type="text" v-model="search" class="form-control searchTopics" placeholder="{{--{{ONE::getStatusTranslation($translations, 'search')}}--}}{{ONE::transCb('cb_list_search', !empty($cb) ? $cb->cb_key : $cbKey)}}">
                        ...
                        <div class="col-12">
                            <div class="grid-view">
                                <div class="container" style="/*height: 100%*/">
                                    <div class="row ideas-list-tile" v-if="showStatistics" style="padding: 0">
                                        <div clasS="col statisticsInformation">
                                            <span>{{ONE::transCb('cb_showing', !empty($cb) ? $cb->cb_key : $cbKey)}}</span> <span>@{{ countTopics }} </span> <span>{{ONE::transCb('cb_of', !empty($cb) ? $cb->cb_key : $cbKey)}}</span> <span>@{{ countTopicsTotal }}</span> <span>{{ONE::transCb('cb_proposals', !empty($cb) ? $cb->cb_key : $cbKey)}}</span>
                                        </div>
                                    </div>
                                    <div v-if="loading_topics" style="text-align: center">
                                        <i class="fa fa-circle-o-notch fa-spin fa-3x fa-fw"></i>
                                    </div>
                                    <transition-group name="topic_block" tag="div" class="row no-gutters" style="display: flex; flex-direction: row;">
                                        <div v-for="topic in filteredTopics" class="topic_block col-12 col-sm-6 col-md-4 idea-card primary-color color-text-primary" :key="topic.topic_key" v-if="topic.visible">
                                            {{--@{{ topic.vote_events }}--}}

                                            <a :href="topic.url" class="a-wrapper">
                                                ...
                                                <div class="idea-details">
                                                    <hr>
                                                    {{--@{{ topic.parameters }}--}}
                                                    <div v-for="topic_parameter in topic.parameters"  class="detail" v-if="topic_parameter.visible_in_list">
                                                        <div v-if="topic_parameter.code == 'numeric'">
                                                            <i class="fa fa-eur" aria-hidden="true" style="margin-right: 5px; "></i>
                                                            <span>@{{ topic_parameter.pivot.value }}</span>
                                                        </div>
                                                        <div v-else-if="topic_parameter._options">
                                                            {{--<strong v-if="topic_parameter._options.length > 0">@{{ topic_parameter.title }}</strong>--}}
                                                            <span v-for="option in topic_parameter._options">@{{ option.title }}</span>
                                                        </div>
                                                        <div v-else>
                                                            <strong>@{{ topic_parameter.title }}</strong>
                                                            <span>@{{ topic_parameter.value }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </a>
                                            ...
                                            <transition-group name="vote_button" tag="div" >
                                                ...
                                                <div v-for="vote in topic.vote_events" class="vote_button" :key="topic.topic_key+vote.vote_key" v-if="vote.is_open">
                                                    {{--@{{ vote.method}}--}}
                                                    {{--<h1>@{{ topic.has_voted[vote.vote_key] }}</h1>--}}
                                                    <div v-if="vote.can_vote && vote.is_open" class="vote_allowed">
                                                        ...
                                                        <div v-else>
                                                            <div v-bind:class="{ voted: topic.has_voted[vote.vote_key] }" class="pointer"  style="text-align: center;padding: 7px 10px" v-on:click="triggerVote(vote,topic,topic.vote_value[vote.vote_key])">
                                                                            <span v-if="topic.has_voted[vote.vote_key]">
                                                                                <span class="" ><?php echo ONE::transCb('cb_voted', $cbKey); ?></span>
                                                                            </span>
                                                                <span v-else>
                                                                                <span class=""{{-- v-on:click="triggerVote(vote,topic,topic.vote_value[vote.vote_key])"--}}><?php echo ONE::transCb('cb_vote', $cbKey); ?></span>
                                                                            </span>
                                                            </div>
                                                        </div>
                                                    </div>
