            ...
            <div class="col-lg-4 col-md-5 col-sm-12 col-12 col-details med-grey-bg">
                <div class="row">
                    ...
                    @if(ONE::checkCBsOption($configurations, 'SHOW-STATUS'))
                        @if(isset($status) && $status->code == 'moderated')
                            <div class="col-12 idea-status">
                                <div class="row no-margin status-row">
                                    ...
                                    <div class="col-6 status-info">{{--{{ONE::getStatusTranslation($translations, $topic->active_status->status_type->code)}}--}}{{ONE::transCb('cb_topic_moderated', !empty($cb) ? $cb->cb_key : $cbKey)}}</div>
                                </div>
                            </div>
                        @else
                            <div class="col-12 idea-status">
                                <div class="row no-margin status-row">
                                    <div class="col-12 text-center">{{--{{ONE::getStatusTranslation($translations, 'topic_detail_status')}}--}}{{ONE::transCb('cb_topic_waiting_moderation', !empty($cb) ? $cb->cb_key : $cbKey)}}</div>
                                </div>
                            </div>
                        @endif
                    @endif
                    <div class="col-12 idea-details">
                        ...
                        <div class="row detail-row">
                            ...
                            <div class="col-6 detail-info">
                                @if(isset($topic->created_on_behalf) && !empty($topic->created_on_behalf))
                                    <?php
                                    $names = explode(';', $topic->created_on_behalf);
                                    if(sizeof($names) > 1){
                                        foreach($names as $name)
                                            echo $name . "<br>";
                                    }else
                                        echo $topic->created_on_behalf;
                        ...
                        @if(isset($parentTopic) && !empty($parentTopic))
                            <div class="row detail-row">
                                <div class="col-6 detail-label" style="color: white!important">{{ ONE::transCb('generated_by', !empty($cb) ? $cb->cb_key : $cbKey) }}</div>
                                <div class="col-6 detail-info">
                                    @if(count($parentTopic) > 1)

                                        @foreach($parentTopic as $pt)
                                            <a style="color:#fff!important;" href="{!! action('PublicTopicController@show', [$pt->cb->cb_key , $pt->topic_key, 'type' => 'proposal'] ) !!}">{!! $pt->title !!}</a>
                                            <br>

                                        @endforeach

                                    @else
                                        <a style="color:#fff!important;" href="{!! action('PublicTopicController@show', [$parentTopic->cb->cb_key , $parentTopic->topic_key, 'type' => 'proposal'] ) !!}">{!! $parentTopic->title !!}</a>
                                    ...
                                    <div class="row no-gutters comment-row">
                                        <div class="col-lg-11 mx-auto">
                                            <div class="input-group comments-input">
                                                ...
                                                <div class="loader float-right" style="display: none;padding-top: 13px;margin-right: 10px;"><img src="{{ asset('images/default/bluePreLoader.gif') }}" alt="Loading" style="width: 20px;"></div>
                                            </div>
                                        </div>
                                    </div>
                                    {{--</div>--}}
