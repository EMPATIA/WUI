...
@section('content')
    <div class="container container-topic">
        ...
        <div class="row" style="text-align: center; color: #aaaaaa; font-size: 12px">
            {{ trans("defaultPadsProject2C.project_2c_number") }}
        </div>
                ...
                @if($isModerator)
                    <a class="btn btn-default btn-md pull-right moderator-buttons darkBtn"
                       href="javascript:oneDelete('{!! action('PublicTopicController@delete', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type]) !!}')" data-toggle="tooltip" data-delay='{"show":"1000"}' data-original-title="Delete">
                        ...
                        <span class="hidden-xs">{!! trans('defaultPadsProject2C.delete') !!}</span>
                    </a>
                    ...
                    <a class="btn btn-default btn-md pull-right moderator-buttons"
                       href="{!! action('PublicTopicController@edit', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type, 'f'=> 'topic']) !!}">
                        ...
                        <span class="hidden-xs">{!! trans('defaultPadsProject2C.change') !!}</span>
                    </a>
                @endif
            </div>
        </div>
                            ...
                            @if(!ONE::isAuth())
                                <div class="alertBoxGreenNews">
                                    <div class="col-xs-12 text-center">
                                        <h2><i class="fa fa-exclamation-circle " aria-hidden="true"></i>  {{ trans("defaultPadsProject2C.login_to_Vote") }}</h2>
                                    </div>
                                </div>
                                            ...
                                            <div class="col-xs-12 col-sm-6">
                                                @if($vt['method'] == 'VOTE_METHOD_MULTI' || $vt['method'] == 'VOTE_METHOD_NEGATIVE')
                                                    <div class="idea-remaining-votes">
                                                        {!! trans('defaultPadsProject2C.remaining_votes') !!}
                                                    </div>
                                                    <div class="idea-vote-counter">
                                                        <div>
                                                            <span class="idea-vote-name-title">{{trans('defaultPadsProject2C.total')}}</span>
                                                            <span id="remainingCounter_{{$vt['key']}}" class="idea-vote-total">{{$vt['remainingVotes']->total}}</span>
                                                        </div>
                                                        <hr class="idea-vote-hr">
                                                        @if($vt['method'] == 'VOTE_METHOD_NEGATIVE')
                                                            <div>
                                                                <i class="fa fa-check" aria-hidden="true" style="width: 14px"> </i> {{ trans('defaultPadsProject2C.positive')}}
                                                                <span id="remainingPositiveCounter_{{$vt['key']}}" class="idea-vote-positive" >{{$vt['remainingVotes']->positive}}</span>
                                                            </div>
                                                            <div>
                                                                <i class="fa fa-times" aria-hidden="true" style="width: 14px"> </i> {{ trans('defaultPadsProject2C.negative')}}
                                                                <span id="remainingNegativeCounter_{{$vt['key']}}" class="idea-vote-negative" >{{$vt['remainingVotes']->positive}}</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                @endif
            ...
            <div class="col-xs-12 col-md-5 contents-margin">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <div class="row">
                            <div class="col-xs-12 project_2c-topic-top-parameter">
                                <i class="fa fa-user"></i>
                                {{isset($usersNames[$topic->created_by]['name']) ? $usersNames[$topic->created_by]['name'] : $topic->created_by}}
                            </div>
                            <div class="col-xs-12 project_2c-topic-top-parameter">
                                @if($statistics["posts_counter"] > 0)
                                    <i class="fa fa-comment-o"></i>{{($statistics["posts_counter"])}} {{ trans("defaultPadsProject2C.comments") }}
                                @endif
                            </div>
                            <div class="col-xs-12 project_2c-topic-top-parameter">
                        ...
                        @foreach($parameters as $parameter)
                            @if($parameter->type->code != 'google_maps')
                                <div class="col-xs-12 project_2c-topic-parameter">
                                    @if($parameter->type->code == 'image_map')
                                        <div class="col-xs-12" id="wrapper">
                                            {!! Form::oneEmpavilleMap( $parameter->id, $parameter->parameter, asset('images/default/parkEmpavilleMapClick.jpg'),false, isset($parameter->pivot->value)? $parameter->pivot->value : null) !!}
                                        </div>
                                    @elseif($parameter->type->code == "category")
                                        <div class="project_2c-parameter-label">
                                            {{$parameter->parameter ?? null}}
                                        </div>
                                        <div class="project_2c-parameter-text">
                                            {{isset($dropDownOptions[$parameter->pivot->value])? $dropDownOptions[$parameter->pivot->value] : null}}
                                        </div>
                                    ...
                                    @elseif($parameter->type->code == "questionnaire")
                                        <a href="{!! action('PublicQController@showQ',$parameter->pivot->value)!!}" target="_blank">
                                            {{ trans("defaultPadsProject2C.form") }}
                                        </a>
                                    @else
                                ...
                                @if( isset($filesByType->images) && sizeof($filesByType->images) > 1 )
                                    <div class="col-sm-12 col-xs-12 col-md-12">
                                        <div class="summary row">
                                            {{trans('defaultPadsProject2C.images')}}
                                        </div>
                                        <div class="project_2c-images-container">
                                            @foreach($filesByType->images as $i=>$fileTmp)
                                                @if($i != 0 && $loop->index < 4)
                                                    <div class="col-sm-4 col-xs-12 thumb project_2c-thumb">
                            ...
                            @foreach($topicData->configurations as $item)
                                ...
                                @if( $item == 'topic_options_allow_files' )
                                    @if( isset($filesByType->docs) )
                                        <div class="summary row">
                                            {{trans('defaultPadsProject2C.files')}}
                                        </div>
                                        <div class="project_2c-files-container">
                                            <div class="well">
                                                @foreach($filesByType->docs as $fileTmp)
                                                    <div class="project_2c-files">
                                ...
                                @if( isset($filesByType->videos) )
                                    <div class="col-sm-12 col-xs-12 col-md-12">
                                        <div class="summary row">
                                            {{trans('defaultPadsProject2C.videos')}}
                                        </div>
                ...
                @if(isset($permissions) && array_key_exists("ALLOW-SHARE", $permissions) && $permissions["ALLOW-SHARE"])
                    <div class="col-xs-12 parameter project_2c-share-btn">
                        <a href="{{ $shareLinks["facebook"]["link"] }}" target="_blank" class="btn-facebook">
                            <i class="fa fa-facebook"></i>
                            {{ trans("defaultPadsProject2C.share") }}
                        </a>
                    </div>
                @endif
            </div>
        ...
        <div class="row">
            <div class="col-xs-12">
                <a class="btn btn-default backButton" href="{!! action('PublicCbsController@show', [$cbKey, 'type'=> $type] ) !!}" >
                    <span>{{trans('defaultPadsProject2C.back_button')}}</span>
                </a>
            </div>
        </div>
        <div class="row">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsProject2C.reportAbuse') !!}</h4>
                </div>
                <div class="modal-body">
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' checked="checked" id="1">{!! trans('defaultPadsProject2C.spam') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="2">{!! trans('defaultPadsProject2C.contains_hate_speech_or_attacks_and_individual') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="3">{!! trans('defaultPadsProject2C.content_not_recommended') !!}</label>
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button type="button" class="btn btn-primary" id="buttonSendAbuse">{!! trans('defaultPadsProject2C.report') !!}</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsProject2C.editPost') !!}</h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" style='margin-left:10px;' class="btn btn-primary pull-right col-sm-2 " id="buttonEditPost">{!! trans('defaultPadsProject2C.updated') !!}</button>
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal"
                            id="frm_cancel">{!! trans('defaultPadsProject2C.close') !!}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsProject2C.postHistory') !!}</h4>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal" id="frm_cancel">{!! trans('defaultPadsProject2C.close') !!}
                    </button>
                </div>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function replyPost(postId, buttonId) {
            ...
            $("#button-append").append("<button class='btn btn-sm btn-primary pull-left btn-flat btn-cancel' id='btn-cancel' type='submit'><?php  echo trans('defaultPadsProject2C.cancel') ?></button>");
