...
@section('content')
    <div class="container container-topic">
        ...
        <div class="row" style="text-align: center; color: #aaaaaa; font-size: 12px">
            {{ trans("defaultPadsProposal.proposal_number") }}
        </div>
                ...
                @if($isModerator)
                    <a class="btn btn-default btn-md pull-right moderator-buttons darkBtn"
                       href="javascript:oneDelete('{!! action('PublicTopicController@delete', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type]) !!}')" data-toggle="tooltip" data-delay='{"show":"1000"}' data-original-title="Delete">
                        ...
                        <span class="hidden-xs">{!! trans('defaultPadsProposal.delete') !!}</span>
                    </a>
                    ...
                    <a class="btn btn-default btn-md pull-right moderator-buttons"
                       href="{!! action('PublicTopicController@edit', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type, 'f'=> 'topic']) !!}">
                        ...
                        <span class="hidden-xs">{!! trans('defaultPadsProposal.change') !!}</span>
                    </a>
                @endif
            </div>
        </div>
                            ...
                            @if(!ONE::isAuth())
                                <div class="alertBoxGreenNews">
                                    <div class="col-xs-12 text-center">
                                        <h2><i class="fa fa-exclamation-circle " aria-hidden="true"></i>  {{ trans("defaultPadsProposal.login_to_Vote") }}</h2>
                                    </div>
                                </div>
                                            ...
                                            <div class="col-xs-12 col-sm-6">
                                                @if($vt['method'] == 'VOTE_METHOD_MULTI' || $vt['method'] == 'VOTE_METHOD_NEGATIVE')
                                                    <div class="idea-remaining-votes">
                                                        {!! trans('defaultPadsProposal.remaining_votes') !!}
                                                    </div>
                                                    <div class="idea-vote-counter">
                                                        <div>
                                                            <span class="idea-vote-name-title">{{trans('defaultPadsProposal.total')}}</span>
                                                            <span id="remainingCounter_{{$vt['key']}}" class="idea-vote-total">{{$vt['remainingVotes']->total}}</span>
                                                        </div>
                                                        <hr class="idea-vote-hr">
                                                        @if($vt['method'] == 'VOTE_METHOD_NEGATIVE')
                                                            <div>
                                                                <i class="fa fa-check" aria-hidden="true" style="width: 14px"> </i> {{ trans('defaultPadsProposal.positive')}}
                                                                <span id="remainingPositiveCounter_{{$vt['key']}}" class="idea-vote-positive" >{{$vt['remainingVotes']->positive}}</span>
                                                            </div>
                                                            <div>
                                                                <i class="fa fa-times" aria-hidden="true" style="width: 14px"> </i> {{ trans('defaultPadsProposal.negative')}}
            ...
            <div class="col-xs-12 col-md-5 contents-margin">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <div class="row">
                            <div class="col-xs-12 proposal-topic-top-parameter">
                                <i class="fa fa-user"></i>
                                {{isset($usersNames[$topic->created_by]['name']) ? $usersNames[$topic->created_by]['name'] : $topic->created_by}}
                            </div>
                            <div class="col-xs-12 proposal-topic-top-parameter">
                                @if($totalComments > 0)
                                    <i class="fa fa-comment-o"></i>{{($totalComments)}} {{ trans("defaultPadsProposal.comments") }}
                                @endif
                            </div>
                            <div class="col-xs-12 proposal-topic-top-parameter">
                        ...
                        @foreach($parameters as $parameter)
                            @if($parameter->type->code != 'google_maps')
                                <div class="col-xs-12 proposal-topic-parameter">
                                    ...
                                    @elseif($parameter->type->code == "category")
                                        <div class="proposal-parameter-label">
                                            {{$parameter->parameter ?? null}}
                                        </div>
                                        <div class="proposal-parameter-text">
                                            {{isset($dropDownOptions[$parameter->pivot->value])? $dropDownOptions[$parameter->pivot->value] : null}}
                                        </div>
                                    ...
                                    @elseif($parameter->type->code == "questionnaire")
                                        <a href="{!! action('PublicQController@showQ',$parameter->pivot->value)!!}" target="_blank">
                                            {{ trans("defaultPadsProposal.form") }}
                                        </a>
                                    @else
                                ...
                                @if( isset($filesByType->images) && sizeof($filesByType->images) > 1 )
                                    <div class="col-sm-12 col-xs-12 col-md-12">
                                        <div class="summary row">
                                            {{trans('defaultPadsProposal.images')}}
                                        </div>
                                        <div class="proposal-images-container">
                                            @foreach($filesByType->images as $i=>$fileTmp)
                                                @if($i != 0 && $loop->index < 4)
                                                    <div class="col-sm-4 col-xs-12 thumb proposal-thumb">
                            ...
                            @foreach($topicData->configurations as $item)
                                ...
                                @if( $item == 'topic_options_allow_files' )
                                    @if( isset($filesByType->docs) )
                                        <div class="summary row">
                                            {{trans('defaultPadsProposal.files')}}
                                        </div>
                                ...
                                @if( isset($filesByType->videos) )
                                    <div class="col-sm-12 col-xs-12 col-md-12">
                                        <div class="summary row">
                                            {{trans('defaultPadsProposal.videos')}}
                                        </div>
                ...
                @if(isset($permissions) && array_key_exists("ALLOW-SHARE", $permissions) && $permissions["ALLOW-SHARE"])
                    <div class="col-xs-12 parameter proposal-share-btn">
                        <a href="{{ $shareLinks["facebook"]["link"] }}" target="_blank" class="btn-facebook">
                            <i class="fa fa-facebook"></i>
                            {{ trans("defaultPadsProposal.share") }}
                        </a>
                    </div>
                @endif
            </div>
        ...
        <div class="row">
            <div class="col-xs-12">
                <a class="btn btn-default backButton" href="{!! action('PublicCbsController@show', [$cbKey, 'type'=> $type] ) !!}" >
                    <span>{{trans('defaultPadsProposal.back_button')}}</span>
                </a>
            </div>
        </div>
        <div class="row">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsProposal.reportAbuse') !!}</h4>
                </div>
                <div class="modal-body">
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' checked="checked" id="1">{!! trans('defaultPadsProposal.spam') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="2">{!! trans('defaultPadsProposal.contains_hate_speech_or_attacks_and_individual') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="3">{!! trans('defaultPadsProposal.content_not_recommended') !!}</label>
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button type="button" class="btn btn-primary" id="buttonSendAbuse">{!! trans('defaultPadsProposal.report') !!}</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsProposal.editPost') !!}</h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" style='margin-left:10px;' class="btn btn-primary pull-right col-sm-2 " id="buttonEditPost">{!! trans('defaultPadsProposal.updated') !!}</button>
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal"
                            id="frm_cancel">{!! trans('defaultPadsProposal.close') !!}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsProposal.postHistory') !!}</h4>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal" id="frm_cancel">{!! trans('defaultPadsProposal.close') !!}
                    </button>
                </div>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function replyPost(postId, buttonId) {
            ...
            $("#button-append").append("<button class='btn btn-sm btn-primary pull-left btn-flat btn-cancel' id='btn-cancel' type='submit'><?php  echo trans('defaultPadsProposal.cancel') ?></button>");
