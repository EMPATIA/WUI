@foreach ($topicsPagination as $topic)
    <a href="{!! action('PublicTopicController@show', [$cb->cb_key , $topic->topic_key, 'type' => $type] ) !!}">
        <div class="col-lg-4 col-md-6 col-xs-12 cbs-boxes-padding">
            <div class="proposal-cbs-box-border">
            ...
            <!-- Files -->
                ...
                <div class="col-xs-12 cbs-box">
                    ...
                    <div class="row topic-contents">
                        <div class="col-xs-12 topic-title proposal-topic-title">
                            <h4>
                                {{$topic->title}}
                            </h4>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-xs-6 col-sm-6">
                                    <div class="topics-ideaDetail">
                                        <i class="fa fa-user" aria-hidden="true"></i> {{ $usersNames->{$topic->created_by}->name ?? trans('defaultCbsProposal.anonymous')}}
                                    </div>
                                    @if(ONE::checkCBsOption($configurations, 'ALLOW-COMMENTS') && $topic->statistics->posts_counter > 0)
                                        <div >
                                            <i class="fa fa-comment-o" aria-hidden="true"></i> {{$topic->statistics->posts_counter}} {{trans('defaultCbsProposal.comments')}}
                                        </div>
                                    @endif
                                </div>
                                @if(!empty($voteType))
        ...
        <div class="col-xs-12">
            @if(isset($filters))
                <a class='jscroll-next'
                   href='{{ URL::action('PublicCbsController@show', ["cbKey"=> $cb->cb_key, "type"=> $type, 'page' => $pageToken, 'filters' => $filters])}}'>{{ trans("defaultCbsProposal.next") }}</a>
            @else
                <a class='jscroll-next'
                   href='{{ URL::action('PublicCbsController@show', ["cbKey"=> $cb->cb_key, "type"=> $type, 'page' => $pageToken])}}'>{{ trans("cbsIdea.next") }}</a>
            @endif
        </div>
