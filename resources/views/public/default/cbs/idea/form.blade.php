    ...
    <div class="container createIdea-container">
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="sub-title">
                    <h2 class="bolder">{{trans("defaultPadsIdea.create_idea")}}</h2>
                </div>
            </div>
        </div>
        ...
        <div class="row form my-form">
            ...
            <div class="container">
                <div class="row">
                    ...
                    {!! Form::oneText('title', trans('defaultPadsIdea.title'), isset($topic) ? $topic->title : null, ['class' => 'form-control', 'id' => 'title', 'required' => 'required']) !!}

                    {!! Form::oneTextArea('summary', trans('defaultPadsIdea.summary'), isset($topic) ? $topic->contents : null, ['class' => 'form-control', 'id' => 'summary', 'size' => '30x2', 'style' => 'resize: vertical', 'required' => 'required']) !!}

                    {!! Form::oneTextArea('contents', trans('defaultPadsIdea.description'), isset($post) ? $post->contents : null, ['class' => 'form-control', 'id' => 'contents', 'size' => '30x10', 'style' => 'resize: vertical', 'required' => 'required']) !!}
                    ...
                    @if((count($parameters) > 0))
                        @foreach($parameters as $parameter)
                            @if($parameter["code"] == 'google_maps')
                                {!! Form::oneMaps('parameter_'.(($parameter["mandatory"]==1) ? "required_" : "").$parameter['id'],trans('defaultPadsIdea.chose_location'),isset($parameter['value'])? $parameter['value'] : null,["required" => $parameter["mandatory"], "defaultLocation" => "38.7436213,-9.1952232", "enableSearch" => true]) !!}
                            @endif
                        @endforeach
                    @endif
                    ...
                    <!-- Files -->
                        @if(isset($configurations) && ((ONE::checkCBsOption($configurations, 'ALLOW-FILES')) || (ONE::checkCBsOption($configurations, 'ALLOW-PICTURES'))))
                            <div class="form-group margin-top" id="attachments-container">
                                {!! ONE::fileSimpleUploadBox("drop-zone", trans("defaultPadsIdea.drag_and_drop_files_to_here") , trans('defaultPadsIdea.add_files'), 'select-files', 'files-list', 'files') !!}
                            </div>
                        @endif
                    </div>
...
@section('scripts')
    ...
    <script>
        $('#topic').submit(function() {
            ...
            if(!isValid){
                toastr.error('{{ trans('defaultPadsIdea.fillInAllRequiredFieldsOnForm') }}', '', {timeOut: 3000,positionClass: "toast-bottom-right"});
                return false;
            }
