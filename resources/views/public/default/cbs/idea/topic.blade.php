...
@section('content')
    <div class="container container-topic">
        ...
        <div class="row" style="text-align: center; color: #aaaaaa; font-size: 12px">
            {{ trans("defaultPadsIdea.idea_number") }}
        </div>
                ...
                @if($isModerator)
                    <a class="btn btn-default btn-md pull-right moderator-buttons darkBtn"
                       href="javascript:oneDelete('{!! action('PublicTopicController@delete', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type]) !!}')" data-toggle="tooltip" data-delay='{"show":"1000"}' data-original-title="Delete">
                        ...
                        <span class="hidden-xs">{!! trans('defaultPadsIdea.delete') !!}</span>
                    </a>
                    ...
                    <a class="btn btn-default btn-md pull-right moderator-buttons"
                       href="{!! action('PublicTopicController@edit', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type, 'f'=> 'topic']) !!}">
                        ...
                        <span class="hidden-xs">{!! trans('defaultPadsIdea.change') !!}</span>
                    </a>
                @endif
            </div>
        </div>
                            ...
                            @if(!ONE::isAuth())
                                <div class="alertBoxGreenNews">
                                    <div class="col-xs-12 text-center">
                                        <h2><i class="fa fa-exclamation-circle " aria-hidden="true"></i>  {{ trans("defaultPadsIdea.login_to_Vote") }}</h2>
                                    </div>
                                </div>
                                            ...
                                            <div class="col-xs-12 col-sm-6">
                                                @if($vt['method'] == 'VOTE_METHOD_MULTI' || $vt['method'] == 'VOTE_METHOD_NEGATIVE')
                                                    <div class="idea-remaining-votes">
                                                        {!! trans('defaultPadsIdea.remaining_votes') !!}
                                                    </div>
                                                    <div class="idea-vote-counter">
                                                        <div>
                                                            <span class="idea-vote-name-title">{{trans('defaultPadsIdea.total')}}</span>
                                                            <span id="remainingCounter_{{$vt['key']}}" class="idea-vote-total">{{$vt['remainingVotes']->total}}</span>
                                                        </div>
                                                        <hr class="idea-vote-hr">
                                                        @if($vt['method'] == 'VOTE_METHOD_NEGATIVE')
                                                            <div>
                                                                <i class="fa fa-check" aria-hidden="true" style="width: 14px"> </i> {{ trans('defaultPadsIdea.positive')}}
                                                                <span id="remainingPositiveCounter_{{$vt['key']}}" class="idea-vote-positive" >{{$vt['remainingVotes']->positive}}</span>
                                                            </div>
                                                            <div>
                                                                <i class="fa fa-times" aria-hidden="true" style="width: 14px"> </i> {{ trans('defaultPadsIdea.negative')}}
                    ...
                    <div class="col-xs-12">
                        ...
                        <div class="row">
                            ...
                            @if(ONE::checkCBsOption($configurations, 'ALLOW-COMMENTS'))
                                <div class="col-xs-12 col-sm-4 idea-topic-top-parameter">
                                    @if($totalComments > 0)
                                        <i class="fa fa-comment-o"></i>{{($totalComments)}} {{ trans("defaultPadsIdea.comments") }}
                                    @endif
                                </div>
                            @endif
                            <div class="col-xs-12 col-sm-4 idea-topic-top-parameter">
        ...
        <div class="row">
            <div class="col-xs-12">
                <a class="btn btn-default backButton" href="{!! action('PublicCbsController@show', [$cbKey, 'type'=> $type] ) !!}" >
                    <span>{{trans('defaultPadsIdea.back_button')}}</span>
                </a>
            </div>
        </div>
        <div class="row">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsIdea.reportAbuse') !!}</h4>
                </div>
                <div class="modal-body">
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' checked="checked" id="1">{!! trans('defaultPadsIdea.spam') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="2">{!! trans('defaultPadsIdea.contains_hate_speech_or_attacks_and_individual') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="3">{!! trans('defaultPadsIdea.content_not_recommended') !!}</label>
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button type="button" class="btn btn-primary" id="buttonSendAbuse">{!! trans('defaultPadsIdea.report') !!}</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsIdea.editPost') !!}</h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" style='margin-left:10px;' class="btn btn-primary pull-right col-sm-2 " id="buttonEditPost">{!! trans('defaultPadsIdea.updated') !!}</button>
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal"
                            id="frm_cancel">{!! trans('defaultPadsIdea.close') !!}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsIdea.postHistory') !!}</h4>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal" id="frm_cancel">{!! trans('defaultPadsIdea.close') !!}
                    </button>
                </div>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function replyPost(postId, buttonId) {
            ...
            $("#button-append").append("<button class='btn btn-sm btn-primary pull-left btn-flat btn-cancel' id='btn-cancel' type='submit'><?php  echo trans('defaultPadsIdea.cancel') ?></button>");
