        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="pads-title">
                    <h3 class="bolder">{{(trans("defaultPadsForum.forum")) }}</h3>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-xs-12">
                <a class="btn btn-default backButton" href="{!! action('PublicCbsController@show', [$cbKey, 'type'=> $type] ) !!}" >
                    <span>{{trans('defaultPadsForum.back_button')}}</span>
                </a>
            </div>
        </div>
        <div class="row" style="text-align: center; color: #aaaaaa; font-size: 12px">
            {{ trans("defaultPadsForum.forum_number") }}
        </div>
                ...
                @if($isModerator)
                    <a class="btn btn-default btn-lg pull-right moderator-buttons"
                       href="javascript:oneDelete('{!! action('PublicTopicController@delete', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type]) !!}')" data-toggle="tooltip" data-delay='{"show":"1000"}' data-original-title="Delete">
                        ...
                        <span class="hidden-xs">{!! trans('defaultPadsForum.delete') !!}</span>
                    </a>
                    <a class="btn btn-default btn-lg pull-right moderator-buttons"
                       href="{!! action('PublicTopicController@edit', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type, 'f'=> 'topic']) !!}">
                        <i class="fa fa-pencil"></i>
                        <span class="hidden-xs">{!! trans('defaultPadsForum.change') !!}</span>
                    </a>
                @endif
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-xs-12 text-muted padding-bottom-15">
                            @if($topicMessage->version != 1)
                                <b><small style="cursor: pointer;" onclick="showTopicHistory('{{$topicMessage->post_key}}')"><i class="fa fa-info"></i> {!! trans('defaultPadsForum.editedAt') !!} {{$topicMessage->updated_at}}</small></b>
                            @else
                                <small>{!! trans('defaultPadsForum.createdBy') !!} {{$topic->created_at}}</small>
                            @endif
                            <div class="text-muted pull-right">
                                @if(ONE::checkCBsOption($configurations, 'ALLOW-REPORT-ABUSE'))
                                    <button type="button" class="btn btn-box-tool grey-btn" id="buttonAbuse_{{$topic->id}}" onclick="reportAbuse('buttonAbuse_{{$topic->first_post->post_key}}', '{{$topic->first_post->post_key}}');">
                                        <i class="fa fa-warning"></i> {!! trans('defaultPadsForum.reportAbuse') !!}
                                    </button>
                                @endif
                            </div>
                        </div>
                                            ...
                                            @if($parameter->type->code != 'google_maps')
                                                <div class="parameter">
                                                    @if($parameter->type->code == 'image_map')
                                                        <div>{{ trans("defaultPadsForum.location") }}</div>
                                                    @else
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsForum.reportAbuse') !!}</h4>
                </div>
                <div class="modal-body">
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' checked="checked" id="1">{!! trans('defaultPadsForum.spam') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="2">{!! trans('defaultPadsForum.contains_hate_speech_or_attacks_and_individual') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="3">{!! trans('defaultPadsForum.content_not_recommended') !!}</label>
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button type="button" class="btn btn-primary" id="buttonSendAbuse">{!! trans('defaultPadsForum.report') !!}</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsForum.editPost') !!}</h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" style='margin-left:10px;' class="btn btn-primary pull-right col-sm-2 " id="buttonEditPost">{!! trans('defaultPadsForum.updated') !!}</button>
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal"
                            id="frm_cancel">{!! trans('defaultPadsForum.close') !!}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">{!! trans('defaultPadsForum.postHistory') !!}</h4>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal" id="frm_cancel">{!! trans('defaultPadsForum.close') !!}
                    </button>
                </div>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function replyPost(postId, buttonId) {
            ...
            $("#button-append").append("<button class='btn btn-sm btn-primary pull-left btn-flat btn-cancel' id='btn-cancel' type='submit'><?php  echo trans('defaultPadsForum.cancel') ?></button>");
