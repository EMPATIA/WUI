    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="sub-title">
                    <h2 class="bolder">{{trans("defaultPadsDiscussion.create_topic")}}</h2>
                </div>
            </div>
        </div>
                ...
                @if((count($parameters) > 0))
                    @foreach($parameters as $parameter)
                        @if($parameter["code"] == 'google_maps')
                            {!! Form::oneMaps('parameter_'.(($parameter["mandatory"]==1) ? "required_" : "").$parameter['id'],trans('defaultPadsDiscussion.chose_location'),isset($parameter['value'])? $parameter['value'] : null,["required" => $parameter["mandatory"], "defaultLocation" => "38.7436213,-9.1952232", "enableSearch" => true]) !!}
                        @endif
                    @endforeach
                @endif
            </div>
            <div class="col-xs-12 parameter">
                {!! Form::oneText('title', trans('defaultPadsDiscussion.title'), isset($topic) ? $topic->title : null, ['class' => 'form-control', 'id' => 'title', 'required' => 'required']) !!}
            </div>
            <div class="col-xs-12 parameter">
                {!! Form::oneTextArea('summary', trans('defaultPadsDiscussion.summary'), isset($topic) ? $topic->contents : null, ['class' => 'form-control', 'id' => 'summary', 'size' => '30x2', 'style' => 'resize: vertical', 'required' => 'required']) !!}
            </div>
            <div class="col-xs-12 parameter">
                {!! Form::oneTextArea('contents', trans('defaultPadsDiscussion.description'), isset($post) ? $post->contents : null, ['class' => 'form-control', 'id' => 'contents', 'size' => '30x10', 'style' => 'resize: vertical', 'required' => 'required']) !!}
            </div>
            <!-- Hidden fields -->
        ...
        <!-- Files -->
            @if(isset($configurations) && ((ONE::checkCBsOption($configurations, 'ALLOW-FILES')) || (ONE::checkCBsOption($configurations, 'ALLOW-PICTURES'))))
                <div class="col-xs-12">
                    <div class="form-group">
                        <label for="title">{{ trans("defaultPadsDiscussion.add_files") }}</label>
                        {!! ONE::fileSimpleUploadBox("drop-zone", trans("defaultPadsDiscussion.drag_and_drop_files_to_here") , trans('defaultPadsDiscussion.files'), 'select-files', 'files-list', 'files') !!}
                    </div>
                </div>
            @endif
            {!! $form->make() !!}
...
@section('scripts')
    ...
    <script>
        $('#topic').submit(function() {
            ...
            if(!isValid){
                toastr.error('{{ trans('defaultPadsDiscussion.fill_in_all_required_fields_on_form') }}', '', {timeOut: 3000,positionClass: "toast-bottom-right"});
                return false;
            }
