...
@section('content')
    <div class="container container-topic">
        ...
        <div class="row" style="text-align: center; color: #aaaaaa; font-size: 12px">
            {{ trans("empavilleNewPadsProposal.idea_number") }}
        </div>
                ...
                @if($isModerator)
                    <a class="btn btn-default btn-md pull-right moderator-buttons darkBtn"
                       href="javascript:oneDelete('{!! action('PublicTopicController@delete', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type]) !!}')" data-toggle="tooltip" data-delay='{"show":"1000"}' data-original-title="Delete">
                        ...
                        <span class="hidden-xs">{!! trans('empavilleNewPadsProposal.delete') !!}</span>
                    </a>
                    ...
                    <a class="btn btn-default btn-md pull-right moderator-buttons"
                       href="{!! action('PublicTopicController@edit', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'type' => $type, 'f'=> 'topic']) !!}">
                        ...
                        <span class="hidden-xs">{!! trans('empavilleNewPadsProposal.change') !!}</span>
                    </a>
                @endif
            </div>
        </div>
                            ...
                            <div style="margin-top: 20px">
                                ...
                                <div class="col-xs-12 col-sm-4 idea-topic-top-parameter">
                                    @if($statistics->posts_counter > 1)
                                        <i class="fa fa-comment-o"></i>{{($statistics->posts_counter - 1)}} {{ trans("empavilleNewPadsProposal.comments") }}
                                    @endif
                                </div>
                                <div class="col-xs-12 col-sm-4 idea-topic-top-parameter">
                            ...
                            @if(!ONE::isAuth())
                                <div class="alertBoxGreenNews">
                                    <div class="col-xs-12 text-center">
                                        <h2><i class="fa fa-exclamation-circle " aria-hidden="true"></i>  {{ trans("empavilleNewPadsProposal.login_to_Vote") }}</h2>
                                    </div>
                                </div>
                                                ...
                                                @if($vt['method'] == 'VOTE_METHOD_MULTI' || $vt['method'] == 'VOTE_METHOD_NEGATIVE')
                                                    <div class="col-xs-12 col-sm-6">
                                                        <div class="idea-remaining-votes">
                                                            {!! trans('empavilleNewPadsIdea.remaining_votes') !!}
                                                        </div>
                                                        <div class="idea-vote-counter">
                                                            <div>
                                                                <span class="idea-vote-name-title">{{trans('empavilleNewPadsIdea.total')}}</span>
                                                                <span id="remainingCounter_{{$vt['key']}}" class="idea-vote-total">{{$vt['remainingVotes']->total}}</span>
                                                            </div>
                                                            <hr class="idea-vote-hr">
                                                            @if($vt['method'] == 'VOTE_METHOD_NEGATIVE')
                                                                <div>
                                                                    <i class="fa fa-check" aria-hidden="true" style="width: 14px"> </i> {{ trans('empavilleNewPadsIdea.positive')}}
                                                                    <span id="remainingPositiveCounter_{{$vt['key']}}" class="idea-vote-positive" >{{$vt['remainingVotes']->positive}}</span>
                                                                </div>
                                                                <div>
                                                                    <i class="fa fa-times" aria-hidden="true" style="width: 14px"> </i> {{ trans('empavilleNewPadsIdea.negative')}}
                        ...
                        <div class="col-xs-12 idea-topic-parameter">
                            ...
                            @elseif($parameter->type->code == "questionnaire")
                                <a href="{!! action('PublicQController@showQ',$parameter->pivot->value)!!}" target="_blank">
                                    {{ trans("empavilleNewPadsProposal.form") }}
                                </a>
                            @else
                        ...
                        @if( isset($filesByType->images) && sizeof($filesByType->images) > 1 )
                            <div class="col-sm-12 col-xs-12 col-md-12">
                                <div class="summary row">
                                    {{trans('empavilleNewPadsProposal.images')}}
                                </div>
                    ...
                    @foreach($topicData->configurations as $item)
                        ...
                        @if( $item == 'topic_options_allow_files' )
                            @if( isset($filesByType->docs) )
                                <div class="summary row">
                                    {{trans('empavilleNewPadsProposal.files')}}
                                </div>
                        ...
                        @if( isset($filesByType->videos) )
                            <div class="col-sm-12 col-xs-12 col-md-12">
                                <div class="summary row">
                                    {{trans('empavilleNewPadsProposal.videos')}}
                                </div>
                ...
                @if(isset($permissions) && array_key_exists("ALLOW-SHARE", $permissions) && $permissions["ALLOW-SHARE"])
                    <div class="col-xs-12 parameter idea-share-btn">
                        <a href="{{ $shareLinks["facebook"]["link"] }}" target="_blank" class="btn-facebook">
                            ...
                            {{ trans("empavilleNewPadsProposal.share") }}
                        </a>
                    </div>
                @endif
            </div>
        ...
        <div class="row">
            <div class="col-xs-12">
                <a class="btn btn-default backButton" href="{!! action('PublicCbsController@show', [$cbKey, 'type'=> $type] ) !!}" >
                    <span>{{trans('empavilleNewPadsProposal.back_button')}}</span>
                </a>
            </div>
        </div>
        <div class="row">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('empavilleNewPadsProposal.reportAbuse') !!}</h4>
                </div>
                <div class="modal-body">
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' checked="checked" id="1">{!! trans('empavilleNewPadsProposal.spam') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="2">{!! trans('empavilleNewPadsProposal.contains_hate_speech_or_attacks_and_individual') !!}</label>
                    </div>
                    <div class="radio">
                        <label><input type="radio" class="optradio" name='optradio' id="3">{!! trans('empavilleNewPadsProposal.content_not_recommended') !!}</label>
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button type="button" class="btn btn-primary" id="buttonSendAbuse">{!! trans('empavilleNewPadsProposal.report') !!}</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">{!! trans('empavilleNewPadsProposal.editPost') !!}</h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" style='margin-left:10px;' class="btn btn-primary pull-right col-sm-2 " id="buttonEditPost">{!! trans('empavilleNewPadsProposal.updated') !!}</button>
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal"
                            id="frm_cancel">{!! trans('empavilleNewPadsProposal.close') !!}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">{!! trans('empavilleNewPadsProposal.postHistory') !!}</h4>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default col-sm-2 pull-right" data-dismiss="modal" id="frm_cancel">{!! trans('empavilleNewPadsProposal.close') !!}
                    </button>
                </div>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function replyPost(postId, buttonId) {
            ...
            $("#button-append").append("<button class='btn btn-sm btn-primary pull-left btn-flat btn-cancel' id='btn-cancel' type='submit'><?php  echo trans('empavilleNewPadsProposal.cancel') ?></button>");
