                ...
                <a href="{!! action('PublicTopicController@show', [$cb->cb_key , $topic->topic_key, 'type' => $type] ) !!}">
                    {{--<div class="row">--}}
                        {{--<div class="col-xs-12 text-right">--}}
                            ...
                            {{--{{\Carbon\Carbon::now()->diffInDays(\Carbon\Carbon::parse($topic->created_at)).' '.trans('empavillePadsIdea.days')}}--}}
                        {{--</div>--}}
                    {{--</div>--}}
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-6 topic-list-bottom">
                                <div class="topics-ideaDetail">
                                    <i class="fa fa-user" aria-hidden="true"></i> {{ $usersNames->{$topic->created_by}->name ?? trans('empavillePadsIdea.anonymous')}}
                                </div>
                                @if(ONE::checkCBsOption($configurations, 'ALLOW-COMMENTS') && $topic->statistics->posts_counter > 0)
                                    <div >
                                        <i class="fa fa-comment-o" aria-hidden="true"></i> {{$topic->statistics->posts_counter ?? 0 }} {{trans('empavillePadsIdea.comments')}}
                                    </div>
                                @endif
                            </div>
                            </a>
                        ...
                        <div class="col-xs-12">
                            <a href="{{ action('AuthController@login') }}">
                                <div class="cb-topics-login-box text-center">
                                    ...
                                    {{ trans("empavillePadsIdea.login_to_vote") }}
                                </div>
                            </a>
                        </div>
                    </div>
    ...
    @if(!empty($pageToken))
        <div class="col-xs-12">
            <a class='jscroll-next'
               href='{{ URL::action('PublicCbsController@show', ["cbKey"=> $cb->cb_key, "type"=> $type, 'page' => $pageToken])}}'>{{ trans("empavillePadsIdea.next") }}</a>
        </div>
