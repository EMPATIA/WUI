                ...
                <div class="box-body" style="margin-top: 10px;background-color: white">
                    @if(count($ideas) > 0)
                        ...
                        @foreach ($ideas as $idea)
                            <div class="col-sm-6 col-md-4 idea" id="proposal_{{$idea->id}}">

                                <div style="margin-bottom: 10px;border-left: 1px solid #f5f5f5; border-top: 3px solid #737373; box-shadow: 4px 4px 5px #d2d6de;">
                                    <div style="padding: 5px; color: #adadad; font-size: 12px">Proposal {{$i++}} of {{count($ideas)}}</div>
                                    ...
                                    <div style="height: 26px; width: 100%; position: relative;">
                                        <h3 style="margin: 5px 0px 0px 10px;overflow: hidden; white-space: nowrap; text-overflow: ellipsis; width: 95%; color: #62a351;">
                                            <a style="font-weight: 600;font-size: 16px; color:#62a351; text-transform: uppercase" href="{!! action('PublicIdeasMessageController@index', $idea->id ) !!}">{{$idea->title}}</a>
                                        </h3>
                                    </div>
                                                ...
                                                @foreach($idea->parameters as $parameter)
                                                    @if($parameter->code != "image_map")
                                                        <div class=" col-xs-offset-1  col-xs-10 col-sm-offset-1 col-sm-10 col-md-offset-1 col-md-10">
                                                            ...
                                                            <span style="color: #62a351;font-weight:bold;">&#62;</span> {{str_replace(array('<br/>', '&', '"'), ' ', $categoriesNameById[$parameter->pivot->value])}}
                                                        </div>
                                                    @else
                                                        <div class=" col-xs-offset-1  col-xs-10 col-sm-offset-1 col-sm-10 col-md-offset-1 col-md-10">
                                    ...
                                    @if($existVotes == 1)
                                        ...
                                        <div class="row">
                                            <div class="col-xs-12 col-sm-12 col-md-12" style="margin-top: 0px; margin-bottom: 15px; text-align: center" >
                                                <a class="btn btn-flat" onclick="voteProposal('{{$idea->id}}', 1)" id="plus_button_{{$idea->id}}" style="<?php echo (array_key_exists($idea->id, $allReadyVoted)) ? ($allReadyVoted[$idea->id] == 1 ? 'background-color: #8cc542;' : 'border: 1px solid #8cc542') : 'border: 1px solid #8cc542;'?>"><i id="vote_plus_{{$idea->id}}" style="<?php echo (array_key_exists($idea->id, $allReadyVoted)) ? ($allReadyVoted[$idea->id] == 1 ? 'color:FFF' : 'color: #8cc542') : 'color: #8cc542'?>" class="fa fa-plus"></i></a>
                                                <a class="btn btn-flat" onclick="voteProposal('{{$idea->id}}', -1)" id="minus_button_{{$idea->id}}" style="<?php echo (array_key_exists($idea->id, $allReadyVoted)) ? ($allReadyVoted[$idea->id] == -1 ? 'background-color: #f74553;' : 'border: 1px solid #f74553') : 'border: 1px solid #f74553;'?>"><i id="vote_minus_{{$idea->id}}" style="<?php echo (array_key_exists($idea->id, $allReadyVoted)) ? ($allReadyVoted[$idea->id] == -1 ? 'color:FFF' : 'color: #f74553') : 'color: #f74553'?>" class="fa fa-minus"></i></a>
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    <div style="border-top: 0px solid #f4f4f4;padding: 10px; text-align: center; background-color: #f4f4f4">
                                        <div class="row">
                                            <div class="col-xs-12 col-sm-12 col-md-12" >
                                                <a  href="{!! action('PublicIdeasMessageController@index', $idea->id ) !!}" style="font-weight: 600;font-size: 14px; text-transform: uppercase; cursor: pointer">Read MORE</a>
                                            </div>
                                        </div>
                                    </div>
