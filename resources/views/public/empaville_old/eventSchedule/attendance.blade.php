...
@if($eventSchedule->type_id == 1)
    @section('content')
    {{ Html::style('/css/empatia/publicScheduleAttendance.css') }}
    <?php
    /* reorder periods */
    $periods = [];
    ...
    ?>
    <div class="box empatia">
        <div class="box-header with-border">
            <h3 class="box-title empatia"><i class="fa"></i> {{ trans('PrivateEventSchedule.schedule') }} - {!! $eventSchedule->title !!}</h3>
        </div>
        <div class="box-body">
       ...
       @if(!isset($_REQUEST["action"]))
                            ...
                            <div class="col-md-4 text-right">
                                <div>
                                    @if(  ( !empty($user) && $user->user_key == $participant->user_key && $eventSchedule->closed == 0) )
                                        <a href="?action=edit&participant_id={{$participant->id}}" title="{{ trans('PrivateEventSchedule.edit') }}" class="btn btn-flat btn-info btn-xs" style="margin-top:6px;" ><i class="fa fa-pencil"></i></a>&nbsp;
            ...
            @if($eventSchedule->closed == 0)
                ...
                <div class="box-footer" style="{{ ($alreadyVoted == true) ? "display:none;": "" }}">
                    <button type="submit" value="form.save" class="btn btn-flat btnSchedule"> {{ trans('PrivateEventSchedule.save') }}</button>
                </div>   
                  ...
                  <tr class="participant">  
                      <td class=" sUserName">
                        ...
                        @if($participant->id == $_REQUEST["participant_id"]  && $eventSchedule->closed == 0 )     
                          ...
                          <div clas="col-md-2">
                                <a href="./{{ $eventSchedule->key }}" title="{{ trans('PrivateEventSchedule.edit') }}" class="btn btn-flat btn-info btn-xs" style="margin-top:6px;margin-right:2px;" ><i class="fa fa-undo"></i></a>&nbsp;
                  ...
                  <tr class="participant">  
                    ...
                    @foreach($eventSchedule->questions as $question)
                        ...
                        <td class="partTableCell y sep pok " title="{{ trans('PrivateEventSchedule.yes') }}">
                            @if(!empty($user) && $user->user_key == $participant->user_key )                            
                                <div class='viewPeriod'>
                                    <i class="glyphicon glyphicon-ok" alt="{{ trans('PrivateEventSchedule.yes') }}"></i>
                                </div>                            
                                <div class='editPeriod' style='display:none;' >
