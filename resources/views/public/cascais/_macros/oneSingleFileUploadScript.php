            ...
            init: {
                ...
                FilesAdded: function (up, files) {
                    ...
                    if (JSON.parse($("#<?php echo $objName; ?>").val()).length<<?php echo $fileCountLimit;?>) {
                        ...
                        showFileUploadError_<?php echo $objName; ?>('upload_file_limit_reached'); ?>');
                ...
                FileUploaded: function (up, file, obj) {
                    ...
                    var jsonString = '[{"id": '+objJson.result.id+', "code": "'+objJson.result.code+'", "name": "'+file.name+'" , "type": "'+file.type+'" , "size": '+file.size+', "description": ""  }]';
                    ...
                    var html = "<div class='fu-file-wrapper'><div id='file_<?php echo $objName; ?>_" +objJson.result.id + "' file_id='" + objJson.result.id + "' class='row'>" +
                        '<div class="col-sm-9 col-12"><span class="fu-file-name" id="file_title_<?php echo $objName; ?>_'+objJson.result.id+'">' + file.name + '</span></div> ' +
                        '<div class="col-sm-3 col-12" style="text-align: right;"> ' +
                        <?php if($isEdit){ ?>
                        '<a class="btn btn-sm file-btn-info" href="javascript:getFileDetails_<?php echo $objName; ?>(' + objJson.result.id  + ')"><i class="fa fa-gear"></i></a> ' +
                        <?php } ?>
                        '<a class="btn btn-sm file-btn-success" href="<?php echo $downloadPath; ?>?id=' + objJson.result.id  + '&code=' + objJson.result.code  +'" target="_blank" ><i class="fa fa-download"></i></a> ' +
                        <?php if($isEdit){ ?>
                        '<a class="btn btn-sm file-btn-danger" href="javascript:removeUploadedFile_<?php echo $objName; ?>('+objJson.result.id +')"><i class="fa fa-trash"></i></a> ' +
                        <?php } ?>
                        '</div> ' +
                        '</div> ' +
                        '</div> ';
                    $("#files-<?php echo $objName; ?>").html(html);
                    $("#file-list").html('<span class="fu-file-name">' + file.name + '</span>');
    ...
    function initFileUploadList_<?php echo $objName; ?>(){
        ...
        for(var i = 0; i < jsonArray.length; i++){
            ...
            var html = "<div class='fu-file-wrapper'><div id='file_<?php echo $objName; ?>_" + jsonArray[i].id + "' file_id='" + jsonArray[i].id + "' class='row'>" +
                '<div class="col-sm-9 col-12"><span class="fu-file-name" id="file_title_<?php echo $objName; ?>_' + jsonArray[i].id + '" >' + jsonArray[i].name + '</span></div> ' +
                '<div class="col-sm-3 col-12" style="text-align: right;"> ' +
                <?php if($isEdit){ ?>
            ...
            $("#file-list").append('<span class="fu-file-name">' + jsonArray[i].name  + '</span>');
