...
@section('content')
    <div class="container container-topic">
        <div class="row">
            <div class="col-xs-12">
                <div class="title">
                    <h2 >{{(trans("defaultPadsForum.forum")) }}</h2>
                </div>
            </div>
        </div>
        <div class="row">
        ...
        <div class="box-body" style="margin-top: 20px">
                ...
                <div class="row">
                    <div class="col-xs-12 text-muted">
                        @if($topicMessage->version != 1)
                            <b><small style="text-decoration: underline; cursor: pointer; margin-left: 20px;" onclick="showTopicHistory('{{$topicMessage->post_key}}')"><i class="fa fa-info" style="color:red"></i> {!! trans('defaultPadsForum.editedAt') !!} {{$topicMessage->updated_at}}</small></b>
                        @else
            ...
            @if(ONE::checkCBsOption($configurations, 'ALLOW-COMMENTS'))
                <div class="col-xs-12 forum-comments">
                    {{ trans("defaultPadsForum.comments") }}
                </div>
                            ...
                            <div class="col-md-2 col-sm-4">
                                <div style="text-align: center;padding-top: 15px;">
                                    ...
                                    <p>
                                        ...
                                        @else
                                            <b>{{trans('defaultPadsForum.anonymous')}}</b>
                                        @endif
                                    </p>
                                </div>
                            </div>
                                ...
                                <div style="border-bottom: 1px solid #f4f4f4;height: 30px; padding-right: 0px; padding-left: 0px;">
                                    <div class="text-muted">
                                        ...
                                        @if($message->version != 1)
                                            <b><small style="text-decoration: underline; cursor: pointer" onclick="showHistory('{{$message->post_key}}')"><i class="fa fa-info" style="color:red"></i> {!! trans('defaultPadsForum.editedAt') !!} {{$message->updated_at}}</small></b>
                                        @else
                                            <small>{!! trans('defaultPadsForum.createdAt') !!} {{$message->created_at}}</small>
                                        @endif
                                        <div class="text-muted pull-right">
                                            @if(ONE::isAuth())
                                                @if(ONE::checkCBsOption($configurations, 'ALLOW-REPORT-ABUSE'))
                                            ...
                                            @if(ONE::getUserKey() == $message->created_by || $isModerator == 1)
                                                <div class="btn-group">
                                                    ...
                                                    <div class="dropdown-menu  pull-right" style="width: 80px !important; min-width: 80px; max-width: 80px;text-align: center; text-align: center;background-color: #fafafa;font-size: 12px;">
                                                        ...
                                                        <a class="dropdown-item" href="javascript:oneDelete('{{ action('PublicPostController@delete', ['cbKey' => $cbKey,'topicKey' => $topicKey, 'postKey' => $message->post_key,'type'=>$type]) }}')">{!! trans('defaultPadsForum.remove') !!}</a>
                                                    </div>
                                                </div>
                                            @endif
                                        </div>
                    ...
                    <form name="topic" accept-charset="UTF-8" method="POST"
                        ...
                        <div class="col-md-10 col-sm-8">
                            ...
                            <div class="box-footer clearfix" style="padding-left: 0px;">
                                <button class="btn btn-sm btn-primary pull-left btn-flat btn-submit" type="submit">{!! trans('defaultPadsForum.submit') !!}</button>
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-4">
                            <div style="text-align: center; height: 200px;padding-top: 35px;">
                                ...
                                <div class="user">
                                    <div>{{Session::has('user') ? Session::get('user')->name : trans('defaultPadsForum.anonymous')}}</div>
                                </div>
                            </div>
                        </div>
                    </form>
