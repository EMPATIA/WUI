...
@section('content')
    <div class="container">
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="sub-title">
                    <h2 >{{trans("defaultPadsProposal.create_proposal")}}</h2>
                </div>
            </div>
        </div>
        <div class="row form">
        ...
        <!-- Form details -->
                ...
                @if((count($parameters) > 0))
                    @foreach($parameters as $parameter)
                        @if($parameter["code"] == 'google_maps')
                            {!! Form::oneMaps('parameter_'.(($parameter["mandatory"]==1) ? "required_" : "").$parameter['id'],trans('defaultPadsProposal.chose_location'),isset($parameter['value'])? $parameter['value'] : null,["required" => $parameter["mandatory"], "defaultLocation" => "38.7436213,-9.1952232", "enableSearch" => true]) !!}
                        @endif
                    @endforeach
                @endif
            </div>
            ...
            <div class="col-xs-12 parameter">
                {!! Form::oneText('title', trans('defaultPadsProposal.title'), isset($topic) ? $topic->title : null, ['class' => 'form-control', 'id' => 'title', 'required' => 'required']) !!}
            </div>
            <div class="col-xs-12 parameter">
                {!! Form::oneTextArea('summary', trans('defaultPadsProposal.summary'), isset($topic) ? $topic->contents : null, ['class' => 'form-control', 'id' => 'summary', 'size' => '30x2', 'style' => 'resize: vertical', 'required' => 'required']) !!}
            </div>
            <div class="col-xs-12 parameter">
                {!! Form::oneTextArea('contents', trans('defaultPadsProposal.description'), isset($post) ? $post->contents : null, ['class' => 'form-control', 'id' => 'contents', 'size' => '30x10', 'style' => 'resize: vertical', 'required' => 'required']) !!}
            </div>
            <!-- Hidden fields -->
        ...
        <!-- Files -->
            @if(isset($configurations) && ((ONE::checkCBsOption($configurations, 'ALLOW-FILES')) || (ONE::checkCBsOption($configurations, 'ALLOW-PICTURES'))))
                <div class="col-xs-12">
                    <div class="form-group">
                        <label for="title">{{ trans("defaultPadsProposal.add_files") }}</label>
                        {!! ONE::fileSimpleUploadBox("drop-zone", trans("defaultPadsProposal.drag_and_drop_files_to_here") , trans('defaultPadsProposal.files'), 'select-files', 'files-list', 'files') !!}
                    </div>
                </div>
            @endif
            {!! $form->make() !!}
...
@section('scripts')
    ...
    <script>
        $('#topic').submit(function() {
            ...
            if(!isValid){
                toastr.error('{{ trans('defaultPadsProposal.fillInAllRequiredFieldsOnForm') }}', '', {timeOut: 3000,positionClass: "toast-bottom-right"});
                return false;
            }
