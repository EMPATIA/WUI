        ...
        <div class="row pads-box">
            @if(empty($cb->end_date) || $cb->end_date >= \Carbon\Carbon::now())
                <div class="col-xs-6 green-text">
                    <span class="glyphicon glyphicon-inbox"></span>{!! trans('defaultCbsDiscussion.discussion_open') !!}
                </div>
                <div class="col-xs-6 green-text text-right">
                    <span class="glyphicon glyphicon-time"></span>{!! trans('defaultCbsDiscussion.close_at') !!}: {{$cb->end_date ?? trans('defaultCbsDiscussion.undefined')}}
                </div>
            @else
                <div class="col-xs-12 gray-text">
                    <span class="glyphicon glyphicon-inbox"></span>{!! trans('defaultCbsDiscussion.discussion_closed') !!}
                </div>
            @endif
            ...
            <div class="col-xs-12">
                <span class="glyphicon glyphicon-time"></span>{!! trans('defaultCbsDiscussion.created_at') !!}: {{\Carbon\Carbon::parse($cb->created_at)->toDateString()}}
                <span class="glyphicon glyphicon-option-vertical"></span>{{$cb->statistics->topics}} {!! trans('defaultCbsDiscussion.topics') !!}
            </div>
        </div>
    </div>
@endforeach
