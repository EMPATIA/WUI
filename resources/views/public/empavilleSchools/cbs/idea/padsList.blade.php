        ...
        <div class="row pads-box">
            @if(empty($cb->end_date) || $cb->end_date >= \Carbon\Carbon::now())
                <div class="col-xs-6 green-text">
                    <span class="glyphicon glyphicon-inbox"></span>{!! trans('empavilleSchoolsPadsIdea.forum_open') !!}
                </div>
                <div class="col-xs-6 green-text text-right">
                    <span class="glyphicon glyphicon-time"></span>{!! trans('empavilleSchoolsPadsIdea.close_at') !!}: {{$cb->end_date ?? trans('empavilleSchoolsPadsIdea.undefined')}}
                </div>
            @else
                <div class="col-xs-12 gray-text">
                    <span class="glyphicon glyphicon-inbox"></span>{!! trans('empavilleSchoolsPadsIdea.forum_closed') !!}
                </div>
            @endif
            ...
            <div class="col-xs-12">
                <span class="glyphicon glyphicon-option-vertical"></span>{{$cb->statistics->topics}} {!! trans('empavilleSchoolsPadsIdea.topics') !!}
            </div>
            <div class="col-xs-12">
                <span class="glyphicon glyphicon-time"></span>{!! trans('empavilleSchoolsPadsIdea.created_at') !!}: {{\Carbon\Carbon::parse($cb->created_at)->toDateString()}}
            </div>
        </div>
    </div>
@endforeach
